...
export async function generateMetadata({
    ...
    const languages: Record<string, string> = {
        pl: `${siteUrl}/pl/jesien`,
        cs: `${siteUrl}/cs/jesien`,
        en: `${siteUrl}/en/jesien`,
        de: `${siteUrl}/de/jesien`,
        nl: `${siteUrl}/nl/jesien`,
    };
    ...
    return {
        ...
        alternates: {
            canonical: `${siteUrl}/${locale}/jesien`,
            languages,
        },
        openGraph: {
            title: t('metaTitle'),
            description: t('metaDescription'),
            url: `${siteUrl}/${locale}/jesien`,
            siteName: 'Zagroda Alpakoterapii',
            locale,
            images: [
                {
                    url: `${siteUrl}/images/activities/forest-walk-opt.jpg`,
                    width: 1200,
                    height: 630,
                    alt: 'Autumn in Zagroda Alpakoterapii',
                },
            ],
        },
    };
...
export default async function AutumnPage({ params: { locale } }: { params: { locale: string } }) {
    ...
    const whatsappUrl = `https://wa.me/48695545330?text=${encodeURIComponent(whatsappText)}`;

    return (
        <div className="bg-stone-950 text-stone-100 min-h-screen">
            {/* Hero Section - optimized padding for above-the-fold mobile visibility */}
            <section className="relative py-12 md:py-20 lg:py-28 overflow-hidden px-4">
                ...
                <div className="relative z-10 max-w-4xl mx-auto text-center mt-2 sm:mt-6 bg-stone-950/75 backdrop-blur-md p-6 sm:p-10 md:p-12 rounded-3xl border border-stone-800/70 shadow-2xl">
                    <span className="inline-block py-1 px-3.5 rounded-full bg-emerald-900/70 text-emerald-400 border border-emerald-700/60 text-xs sm:text-sm font-bold mb-4 sm:mb-5">
                        {t('badge')}
                    </span>

                    <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-stone-50 mb-4 sm:mb-5 tracking-tight drop-shadow-md leading-tight">
                        {t('heroTitle')}
                    </h1>

                    {/* Location Hook */}
                    <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-500/20 border border-amber-400/40 text-amber-300 text-xs sm:text-sm font-semibold mb-4 sm:mb-6 shadow-sm">
                        <span>📍</span>
                        <span>{t('locationHook')}</span>
                    </div>

                    <p className="text-base sm:text-xl text-stone-200 mb-6 sm:mb-8 max-w-2xl mx-auto leading-relaxed drop-shadow">
                        {t('heroSubtitle')}
                    </p>

                    {/* Friction Reducers Bar */}
                    <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mb-6 sm:mb-8 text-xs sm:text-sm">
                        <span className="inline-flex items-center gap-1.5 bg-stone-800/80 border border-stone-700/80 text-emerald-400 px-3 py-1.5 rounded-full font-medium shadow-sm">
                            <span>🇨🇿</span>
                            {t('frictionLanguage')}
                        </span>
                        <span className="inline-flex items-center gap-1.5 bg-stone-800/80 border border-stone-700/80 text-stone-200 px-3 py-1.5 rounded-full font-medium shadow-sm">
                            <span>💳</span>
                            {t('frictionPayment')}
                        </span>
                        <span className="inline-flex items-center gap-1.5 bg-emerald-950/80 border border-emerald-500/40 text-emerald-300 px-3 py-1.5 rounded-full font-bold shadow-sm">
                            <span>🏷️</span>
                            {t('frictionPromo')}
                        </span>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
                        <Link
                            href={`/${locale}/stay?code=${promoCode}`}
                            className="bg-emerald-700 hover:bg-emerald-600 text-white px-7 sm:px-8 py-3.5 sm:py-4 rounded-xl font-bold transition-all text-base sm:text-lg shadow-lg hover:shadow-emerald-900/50 w-full sm:w-auto text-center transform hover:scale-[1.02] active:scale-[0.98]"
                        >
                            {t('ctaBook')}
                        </Link>
                        <a
                            ...
                            className="inline-flex items-center justify-center gap-2.5 bg-[#25D366] hover:bg-[#20ba5a] text-white px-7 sm:px-8 py-3.5 sm:py-4 rounded-xl font-bold transition-all text-base sm:text-lg shadow-lg hover:shadow-green-900/50 w-full sm:w-auto transform hover:scale-[1.02] active:scale-[0.98]"
                        >
                        ...
                        <Link
                            href="#activities"
                            className="bg-stone-800/90 hover:bg-stone-700 text-stone-200 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl font-medium transition-colors border border-stone-700 text-base sm:text-lg w-full sm:w-auto text-center"
                        >
                            {t('ctaActivities')}
                        </Link>
                    </div>
            ...
            <section className="py-20 bg-stone-900 border-t border-stone-800 px-4">
                <div className="max-w-4xl mx-auto bg-stone-950 p-8 rounded-2xl border border-stone-800">
                    ...
                    <div className="mt-8 text-center flex flex-col sm:flex-row gap-4 justify-center items-center">
                        <Link href={`/${locale}/stay?code=${promoCode}`} className="inline-block bg-emerald-700 hover:bg-emerald-600 text-white px-6 py-3 rounded-lg font-bold transition-colors shadow-md">
                            {t('checkAvailability')}
                        </Link>
                        <a
                            href={whatsappUrl}
